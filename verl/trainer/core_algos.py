...
@register_adv_estimator(AdvantageEstimator.SGPO)
def compute_sgpo_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    scores = token_level_rewards.sum(dim=-1)
    id2score = defaultdict(list)
...
def average_loss(
    values: torch.Tensor, 
    mask: torch.Tensor, 
    mode: Literal["token", "seq", "sgpo"],  # 添加 "sgpo"
    eps: float = 1e-8,
    sgpo_max_tokens: int = None,  # 新增参数
) -> torch.Tensor:
    if mode == "token":
        return VF.masked_mean(values, mask, eps=eps)
    
    elif mode == "sgpo":
        per_sequence_sum_loss = (values * mask).sum(-1)
        # mask.sum(-1)指的是每个序列中有效token的数量
        print(f"Debug: mask.sum(-1) = {mask.sum(-1)}")
        normalized_loss = per_sequence_sum_loss / (sgpo_max_tokens if sgpo_max_tokens else 90)
        # normalized_loss = per_sequence_sum_loss / (sgpo_max_tokens if sgpo_max_tokens else mask.sum(-1) + eps)
        return normalized_loss.mean()
    
    elif mode == "seq":
        # 原来的 seq 模式逻辑（如果需要的话）
        per_seq_mean = (values * mask).sum(-1) / (mask.sum(-1) + eps)
...
def compute_policy_loss(
    ...
    loss_avg_mode: Literal["token", "seq","sgpo"],
    ...
    Args:
        ...
        beta: (float)
            The coefficient for KL divergence in the policy gradient loss. This is used to balance the policy
            gradient loss and the KL divergence term.
    Returns:
    ...
    sgpo_max_tokens: int = None
    # pg metrics
    metrics = {"ppo_kl": -negative_approx_kl}
    # use negative log probs as an estimator of entropy loss
    metrics["entropy_loss"] = average_loss(-log_probs, response_mask, mode=loss_avg_mode, sgpo_max_tokens=sgpo_max_tokens)

    pg_loss = -advantages * ratio  # -ratio * A
    pg_loss2 = -advantages * clipped_ratio  # -clip(ratio, 1-clip_low, 1+clip_high) * A
    # pg_loss3 = -advantages * clip_ratio_dual  # modified: dual-clip disabled

    final_pg_loss = torch.max(pg_loss, pg_loss2)
    # pg_clipfrac_higher 的意思是 final_pg_loss 来自 pg_loss2 的比例
    # pg_clipfrac_lower  的意思是 final_pg_loss 来自 pg_loss3 的比例
    metrics["pg_clipfrac_higher"] = (pg_loss < pg_loss2).float()
    # metrics["pg_clipfrac_lower"] = (final_pg_loss > pg_loss3).float() * (advantages < 0).float()  # modified: dual-clip disabled
    metrics["pg_clipfrac_lower"] = torch.zeros_like(metrics["pg_clipfrac_higher"])  # modified: placeholder when dual-clip disabled
    final_pg_loss = average_loss(final_pg_loss, response_mask, mode=loss_avg_mode, sgpo_max_tokens=sgpo_max_tokens)
...
def compute_value_loss(
    ...
    sgpo_max_tokens=None
    vf_loss = 0.5 * average_loss(clipped_vf_losses, response_mask, mode=loss_avg_mode, sgpo_max_tokens=sgpo_max_tokens)
    metrics = {
