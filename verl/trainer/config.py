...
#     active_learning: ActiveLearningConfig = field(default_factory=ActiveLearningConfig) # <--- 添加此行
#
...
class AlgorithmConfig:
    ...
    adv_estimator: str = "sgpo"
    """advantage estimator, support `gae`, `grpo`, `reinforce_plus_plus`, `remax`, `rloo`, `sgpo`"""
...
class TrainerConfig:
    ...
    def post_init(self):
        if self.save_checkpoint_path is None:
            ...
            self.save_checkpoint_path = os.path.join("/path/to/checkpoint/saves", self.project_name, f"{self.experiment_name}")
...
class ActiveLearningConfig:
    total_rounds: int = 5
    k_samples: int = 8  # 每次推理采样k个样本
    sample_size: int = 300 # 每轮选出N个高熵样本
